...
when available. Rofi configuration parsing runs when Rofi and DISPLAY are
available. The secret scan is a basic guard, not proof of absence of secrets.
...
partition jobs or writes a block device. See `build/README.md` for the
VM installation matrix and recorded evidence; these checks cannot establish bootloader correctness.
